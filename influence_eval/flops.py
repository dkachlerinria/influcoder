...
def flops_influcoder(
    *,
    ...
    """Influcoder: gradient stocking + encoder training + encoder inference.
    ...
    Encoder training cost: pass as `training_flops` (measured by
    train_influence_encoder.py and stored in _flops.json).
    Encoder inference cost: forward-only for the Spearman eval set.

    The primary path is `measured_flops` in params.pt (stocking + training +
    inference all captured by FlopCounterMode). This analytic fallback is only
    used for old runs that predate measurement.
    """
    stocking = flops_less(
    ...
    return stocking + int(training_flops) + inference
...
def flops_for_method(method: str, params: dict, seq_len: int = 2048) -> dict:
    ...
    # For multi-phase methods (iprox, influcoder) measured_flops = total (train+inf),
    # while inference_flops holds the scoring-only number.
    ...
    if method in ("ground_truth", "less", "less_small"):
        ...
        # Analytic fallback for old runs (no measured_flops). Training FLOPs
        # are not known analytically so only scoring cost is estimated here.
        ...
        training = int(params.get("training_flops", 0))
        out["inference"] = int(scoring)
        out["total"] = int(scoring) + training
    elif method == "influcoder":
        # Total = Stocking + Training + Inference
        # Inference = Encoder inference on anchors + pool only
        inf = flops_embedding(
            num_params_encoder=params["total"],
            seq_len=seq_len,
            num_anchors=params["num_anchors"],
        ...
        tot = flops_influcoder(
            ...
            num_train=params["num_train"],
            emb_dim=params["emb_dim"],
            training_flops=params.get("training_flops", 0),
        )
