...
def _storage_bytes(params: dict) -> int:
    """Bytes needed to store one sample's influence representation (float32)."""
    for key in ("emb_dim", "proj_dim", "grad_dim"):
        if key in params:
            return int(params[key]) * 4
    return 0


def _markdown_table(results: dict) -> str:
    lines = [
        "| method | per-anchor mean | per-anchor std | agg(mean) | agg(max) | Total FLOPS | Inf. FLOPS | Inf. Time (s) | Time/Sample (ms) | Bytes/sample | KB/1K samples |",
        "|---|---|---|---|---|---|---|---|---|---|---|",
    ]
    ...
    for method in methods:
        ...
        results["methods"][method] = {
            **metrics,
            ...
            "kb_per_1k_samples": bps * 1000 / 1024,
            "params_meta": {
