...
# NOTE (influcoder port): the original file also imported `datasets.load_dataset`,
# `functorch.{grad,make_functional_with_buffers,vmap}`, and `common.data.*`. The
# functorch symbols were unused; the datasets/common.data ones are only touched
# by the standalone-script data loaders (load_train_dataset / compute_eval_grads
# / __main__), which this port does not use -- it feeds pre-tokenized influcoder
# Samples straight into collect_grads. Dropping them keeps the module importable
# without those (removed/renamed) dependencies. collect_grads itself is verbatim.
...
def get_trak_projector(device: torch.device):
    ...
    Returns (projector_class, uses_custom_cuda).
    uses_custom_cuda=True  → CudaProjector (fast_jl custom kernel, NOT counted by
                              FlopCounterMode — caller must add projection FLOPs manually).
    uses_custom_cuda=False → BasicProjector (standard PyTorch mm/bmm, IS counted by
                              FlopCounterMode — caller must NOT add projection FLOPs manually).
    """
    try:
...
def collect_grads(
    ...
    projector, uses_custom_cuda = get_trak_projector(device)
    ...
    return torch.cat(projected_grads, dim=0), uses_custom_cuda
