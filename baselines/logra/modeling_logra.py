...
class LoGra:
    ...
    def __init__(
        ...
        Args:
            ...
            mlp_only: If True, only apply LoRA to MLP layers (ignored when target_modules is set)
            target_modules: Explicit list of module name suffixes to target (e.g. ['q_proj', 'v_proj']).
                            When set, takes precedence over mlp_only.
            torch_dtype: Model dtype
            device: Device to use
            attn_implementation: "sdpa" (default) is far more memory-efficient
                than "eager" for backprop (no explicit [seq,seq] attention
                matrix materialized per head/layer) -- pass "eager" only when
                also running under `baselines.cost.flop_counter`, since
                torch.utils.flop_counter's SDPA handler crashes on GQA models
                (Qwen3 is GQA). Safe to leave at "sdpa" for plain timing runs.
        """
        self.device = device
    ...
    def from_pretrained(
        ...
        Args:
            ...
            mlp_only: Only apply LoRA to MLP layers (ignored when target_modules is set)
            target_modules: Explicit list of module name suffixes to target.
            torch_dtype: Model dtype
            device: Device to use
            attn_implementation: see LoGra.__init__
        """
        return cls(
