...
python3 -m influence_eval.compute_influcoder_scores \
    --encoder_dir          "${INFLUCODER_ENCODER_DIR_68M}/model" \
    --gradient_model       "${INFLUENCE_MODEL}" \
    --save_dir             "${INFLUENCE_OUT}" \
    --out_name             "influcoder_68m" \
    ...
    --training_flops_path  "${INFLUCODER_ENCODER_DIR_68M}/_flops.json" \
    --stocking_timing_path "${INFLUCODER_DB_DIR}/_timing.json" \
    --training_timing_path "${INFLUCODER_ENCODER_DIR_68M}/_timing.json"
