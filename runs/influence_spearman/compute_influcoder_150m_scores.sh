...
python3 -m influence_eval.compute_influcoder_scores \
    --encoder_dir          "${INFLUCODER_ENCODER_DIR_150M}/model" \
    --gradient_model       "${INFLUENCE_MODEL}" \
    --save_dir             "${INFLUENCE_OUT}" \
    --out_name             "influcoder_150m" \
    --train_texts_path     "${INFLUENCE_OUT}/data/eval_pool_texts.json" \
    --anchor_texts_path    "${INFLUENCE_OUT}/data/eval_anchors_texts.json" \
    ...
    --training_flops_path  "${INFLUCODER_ENCODER_DIR_150M}/_flops.json" \
    --stocking_timing_path "${INFLUCODER_DB_DIR}/_timing.json" \
    --training_timing_path "${INFLUCODER_ENCODER_DIR_150M}/_timing.json"
