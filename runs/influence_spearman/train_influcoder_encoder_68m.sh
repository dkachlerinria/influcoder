...
mkdir -p "${INFLUCODER_ENCODER_DIR_68M}"
...
python influcoder/train_influence_encoder.py \
    ...
    --encoder_model       "${ENCODER_MODEL_68M}" \
    --gradient_model      "${INFLUENCE_MODEL}" \
    --run_mode            "${INFLUCODER_RUN_MODE}" \
    --output_dir          "${INFLUCODER_ENCODER_DIR_68M}" \
    --lora_rank           "${LORA_RANK}" \
    ...
    --eval_gt
echo "⏱  Encoder-68m training total time: $((SECONDS - SCRIPT_START))s"
