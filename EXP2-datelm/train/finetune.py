...
from methods.select_data import select
...
def setup(
    ...
    metric_path: Optional[str] = None,  # Add metric_path parameter
) -> None:
    ...
    Arguments:
        ...
        metric_path: Path to the metric file for data selection.
    """
...
def main(
    ...
    train_dataloader, val_dataloader = get_dataloaders(fabric, data, tokenizer, train, metric_path)
...
def get_dataloaders(
    fabric: L.Fabric, data: DataModule, tokenizer: Tokenizer, train: TrainArgs, metric_path: Optional[str]
) -> Tuple[DataLoader, DataLoader]:
    with fabric.rank_zero_first():
        data.prepare_data()
    data.setup()

    selected_indices = select(
        metric_path=metric_path,  # Pass metric_path here
        metrics_format="npy",
        select_from_size=200000,
        selection_size=10000,
        gumbel_temp=None,  # instruction tuning default doesn't apply gumbel
    )
    # Filter train dataset
    data.select_train_subset(selected_indices)
