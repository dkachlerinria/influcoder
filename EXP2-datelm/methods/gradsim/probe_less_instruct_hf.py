...
import numpy as np
from train.model_utils import checkpoints_load_func
...
def setup(
    ...
    avg, avg_sq = prepare_optimizer_state_hf(model, adam_optimizer_state, "cuda")

    print(f"Loaded model and optimizer")
    ...
    for cnt, train_data in enumerate(tqdm(train_iterator), start=1):
        scores = fit(device, {"model": model}, train_data, val_dataloader, avg, avg_sq)
    ...
    print("output_array", output_array[:100])
...
def fit(
    ...
    avg_sq: Optional[torch.Tensor] = None,
) -> torch.Tensor:
    model = state["model"]
    val_iter = iter(val_dataloader)
    return gradient_cos(device, train_data["input_ids"].to('cuda'), train_data["labels"].to('cuda'), model, val_iter, avg, avg_sq)


def gradient_cos(device, input_ids, labels, model, val_iter, avg=None, avg_sq=None):
    def collect_grad(model):
        return torch.cat([p.grad.view(-1) for p in model.parameters() if p.grad is not None])
    
    def collect_grad_adam(model, avg, avg_sq):
    ...
    for i, sample_loss in enumerate(perseq_loss):
        ...
        # sample_grad = collect_grad(model) # SGD grads
        sample_grad = collect_grad_adam(model, avg, avg_sq) # for LESS, train data uses adam optimizer-aware gradients. 
        # it seems that for LESS, fewer similarity scores are positive? maybe only 5%, whereas for vanilla, 50% are positive.
