#!/usr/bin/env python3
"""More-external-samples ablation of `influcoder_attribute_noleak_counterfact_extquery.py`
-- ONLY N_EXT_ANCHORS/N_TEACHER_TRAIN/N_EVAL_TRAIN are raised (300->900,
500->2000, 250->500); HARD_RATIO and EPOCHS are held fixed at their original
values on purpose. FINDINGS.md (EXP1, same distill() code path) found that
scaling up data WITHOUT proportionally raising hard_ratio can regress quality
(pool dilutes easy-negative signal faster than it adds hard-negative signal) --
this run deliberately does NOT compensate for that, to test whether the effect
shows up here too rather than assuming it transfers. See EXP2.md for the result.
...
MAX_LEN = 1024
PROJ_DIM = 8192
N_EXT_ANCHORS = 900   # moredata ablation: 3x the 300 used in the recommended run
N_TEACHER_TRAIN = 2000  # moredata ablation: 4x the 500 used in the recommended run
N_EVAL_TRAIN = 500     # moredata ablation: 2x the 250 used in the recommended run
...
SAVE_PATH = Path("results/factual-attribution-influcoder-noleak-extquery-moredata/InfluCoder.pt")
