...
CROSS-SOURCE toxic data: anchors and candidates deliberately come from two
DIFFERENT toxic corpora, so the anchor<->candidate gradient-similarity signal
the encoder learns can't just be "these two texts share the same corpus's
writing style/format" -- it has to be an actual toxicity-content signal that
generalizes across sources. Same moredata sample counts as
`influcoder_attribute_noleak_toxicity_wildguard_moredata.py` and
`influcoder_attribute_noleak_toxicity_moredata.py`, HARD_RATIO/EPOCHS held
fixed as always.
...
Role assignment (stated plainly, not hidden):
  ...
  - toxic held-out eval (100): WildGuardMix, disjoint from the anchor slice
    (same source as anchors, not candidates -- eval's role here is closer to
    "another query-side probe for best-epoch selection" than to the scored
    candidate pool, so sourcing it like the anchors is the more defensible
    read of "what is this held-out set standing in for," not an attempt to
    make the eval number look better by matching it to whichever source is
    "easier")
  - benign candidates/eval: UltraChat throughout, unchanged from every other
    toxicity script in this doc

This is the test for whether the single-source design of every other
toxicity script here (`_toxicity.py`, `_toxicity_moredata.py`,
`_toxicity_wildguard.py`, `_toxicity_wildguard_moredata.py` -- all draw
anchors AND candidates from the same shuffled pool of one corpus) was a real
confound inflating those AUPRC numbers via anchor/candidate stylistic
similarity rather than toxicity-content similarity, or not.

Usage:
    python methods/influcoder_attribute_noleak_toxicity_mixed.py
"""
...
MAX_LEN = 1024
PROJ_DIM = 8192
N_EXT_ANCHORS = 100     # WildGuardMix -- same count as the moredata runs
...
SAVE_PATH = Path("results/toxicity-bias-influcoder-noleak-mixed/InfluCoder.pt")
...
def main():
    random.seed(SEED)
    print(f"InfluCoder (leak-free, CROSS-SOURCE mixed) / DATE-LM | task={TASK} subset={SUBSET} "
