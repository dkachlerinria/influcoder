...
`HARD_RATIO` 0.5 -> 0.0 (no hard mining at all). Sample counts, epochs, seed,
and every other constant/piece of logic are byte-identical to that script.

This isolates the hard_ratio variable cleanly at the ORIGINAL (smaller,
already-proven) sample counts -- distinct from
`influcoder_attribute_noleak_toxicity_mixed_bigger.py`, which tested
hard_ratio=0.0 combined WITH a 3-5x sample-count scale-up and regressed to
0.5191. This run tells us whether that regression was really about
hard_ratio, or about the scale-up, or both.
...
Usage:
    python methods/influcoder_attribute_noleak_toxicity_mixed_nohard.py
"""
...
HARD_RATIO = 0.0        # ONLY CHANGE vs. the 0.6160 winning script (was 0.5) -- explicit instruction
LR = 5e-5
SEED = 0
SELECT_BEST_ON = "aggregated"

SAVE_PATH = Path("results/toxicity-bias-influcoder-noleak-mixed-nohard/InfluCoder.pt")
...
def main():
    random.seed(SEED)
    print(f"InfluCoder (leak-free, CROSS-SOURCE mixed, NO HARD MINING) / DATE-LM | task={TASK} subset={SUBSET} "
          f"| base_model={BASE_MODEL} checkpoint={CHECKPOINT}\n")
    print("toxic anchors <- WildGuardMix | toxic candidates <- ToxicChat | "
          "toxic held-out eval <- WildGuardMix (disjoint from anchors) | hard_ratio=0.0\n")
