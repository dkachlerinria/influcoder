...
(XSTest-response-Het subset) -- identical to
`influcoder_attribute_noleak_toxicity.py` except the toxic-side external
source is **WildGuardMix** (`allenai/wildguardmix`) instead of
`lmsys/toxic-chat`. This closes the distributional-match gap flagged in
EXP2.md §4.2/§8: WildGuardMix is built by the same WildGuard team as the
local task's own source (`allenai/xstest-response`), has response-level harm
labels (`response_harm_label`) matching `xstest-response`'s
`response_harmfulness` semantics directly (ToxicChat only has prompt-level
`toxicity`), and has far more headroom (8,368 usable harmful-response rows
in the `wildguardtrain` config vs. ToxicChat's 746 usable toxic rows).

Schema of `wildguardtrain` (86,759 rows): `prompt`, `response`,
`prompt_harm_label` ('harmful'/'unharmful'), `response_refusal_label`
('refusal'/'compliance', None on ~48.8k prompt-only rows with no response
scored), `response_harm_label` ('harmful'/'unharmful', None on those same
~48.8k rows), `adversarial` (bool), `subcategory`. The toxic pool here is
rows with `response_harm_label == "harmful"` and a non-empty `response` --
the direct analog of `xstest-response`'s harmful-response rows.

Same sample counts, same hard_ratio/epochs as the ToxicChat-sourced
recommended script (N_EXT_ANCHORS=40/N_EXT_BENIGN=300/N_EXT_TOXIC_POS=100/
N_EVAL_BENIGN=100/N_EVAL_TOXIC=50/EPOCHS=8/HARD_RATIO=0.5) -- this is meant
as a like-for-like swap of the external source, directly comparable to that
script's result, not a re-tuned config.

Usage:
    python methods/influcoder_attribute_noleak_toxicity_wildguard.py
"""
...
N_EXT_TOXIC_POS = 100   # external toxic candidates (fresh WildGuardMix)
N_EVAL_BENIGN = 100
N_EVAL_TOXIC = 50
EPOCHS = 8
HARD_RATIO = 0.5
...
SAVE_PATH = Path("results/toxicity-bias-influcoder-noleak-wildguard/InfluCoder.pt")
...
def main():
    random.seed(SEED)
    print(f"InfluCoder (leak-free, WildGuardMix) / DATE-LM | task={TASK} subset={SUBSET} "
