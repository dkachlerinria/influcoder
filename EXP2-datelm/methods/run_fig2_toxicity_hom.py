#!/usr/bin/env python3
"""Unified Fig 2 (Toxicity/Bias, Hom variant) timing + accuracy harness --
Homogeneous analog of `run_fig2_toxicity.py` (which runs Heterogeneous/Het).
Same file, same design, same 9 methods, same fixes already applied there
(np.asarray() before .mean(axis=1) for influcoder.encoder.embed()'s numpy
return type -- see run_fig2_toxicity.py's run_influcoder() for the story).

Het vs Hom is a real structural difference in the local data, not just a
different checkpoint: Het's local train (10,187) = 10,000 Benign + 66 Unsafe
+ 121 Safety-Aligned (two distinct planted-unsafe categories, mixed). Hom's
local train (10,066) = 10,000 Benign + 66 Unsafe ONLY -- no Safety-Aligned
category at all. "Homogeneous" = single planted-unsafe type; "Heterogeneous"
= two distinct types mixed in. Confirmed directly via get_dataset() before
writing this script, not assumed from the naming alone.

InfluCoder's config here is the SAME hyperparameters as the established-best
Het config (WildGuardMix anchors+eval / ToxicChat candidates, ettin-400m,
hard_ratio=0.5) -- untuned for Hom specifically, since no Hom-specific sweep
has been run. Treat this as a reasonable carry-over starting point, not a
verified-best-for-Hom config the way the Het one is.

Methods: bm25, repsim, graddot, gradsim, less, datainf, ekfac, influcoder, semantic.

Usage:
    python methods/run_fig2_toxicity_hom.py --method bm25
    (... same --method choices as run_fig2_toxicity.py)
"""
...
# ---------------------------------------------------------------------------
# Harmless import-time-only stubs for litgpt/lightning -- see
# run_fig2_counterfact.py's docstring for the full rationale.
# ---------------------------------------------------------------------------
...
# ============================================================================
# ONE PLACE for every method's parameters. Only TASK/SUBSET/CHECKPOINT/
# RESULTS_DIR/SUMMARY_PATH differ from run_fig2_toxicity.py's Het version.
# ============================================================================
TASK = "Toxicity/Bias"
SUBSET = "XSTest-response-Hom"
BASE_MODEL = "EleutherAI/pythia-1b"
CHECKPOINT = "DataAttributionEval/Pythia-1b-XSTest-response-Hom"
MAX_LEN = 1024
SEED = 0
...
# InfluCoder: same hyperparameters as the established-best Het config --
# NOT independently tuned for Hom. See module docstring.
...
def run_bm25():
    ...
    flat_scores = scores.mean(axis=0)  # [n_train]
    wall_s = time.perf_counter() - t0

    save_path = RESULTS_DIR / "fig2-toxicity-hom-bm25" / "BM25.pt"
...
def run_repsim():
    ...
    save_path = RESULTS_DIR / "fig2-toxicity-hom-repsim" / "Rep_Sim.pt"
    save_path.parent.mkdir(parents=True, exist_ok=True)
    with open(save_path, "w") as f:
        json.dump(flat_scores.tolist(), f)
...
def run_dattri(method_key: str):
    ...
    save_dir = RESULTS_DIR / f"fig2-toxicity-hom-{method_key}"
...
# ============================================================================
# InfluCoder -- cross-source mix (WildGuardMix anchors+eval, ToxicChat
# candidates), same hyperparameters as the established-best Het config.
# ============================================================================
def run_influcoder():
    ...
    flat_scores = np.asarray(scores).mean(axis=1)  # [n_train] -- see run_fig2_toxicity.py's note
    inference_s = time.perf_counter() - t_inf0

    save_path = RESULTS_DIR / "fig2-toxicity-hom-influcoder" / "InfluCoder.pt"
    save_path.parent.mkdir(parents=True, exist_ok=True)
    with open(save_path, "w") as f:
        json.dump(flat_scores.tolist(), f)
...
def run_semantic():
    ...
    # load_encoder(), which InfluCoder's inference timer excludes.
    t_setup0 = time.perf_counter()
    local_train, local_ref = get_dataset(TASK, SUBSET)
    enc = load_encoder(INFLUCODER_ENCODER_MODEL, max_seq_len=INFLUCODER_ENCODER_MAX_LEN)
    setup_s = time.perf_counter() - t_setup0
    ...
    flat_scores = np.asarray(scores).mean(axis=1)
    wall_s = time.perf_counter() - t_inf0

    save_path = RESULTS_DIR / "fig2-toxicity-hom-semantic" / "Semantic.pt"
    save_path.parent.mkdir(parents=True, exist_ok=True)
    with open(save_path, "w") as f:
        json.dump(flat_scores.tolist(), f)
...
def main():
    ...
    print(f"=== Fig 2 (Toxicity/Bias/XSTest-response-Hom): {args.method} ===")
