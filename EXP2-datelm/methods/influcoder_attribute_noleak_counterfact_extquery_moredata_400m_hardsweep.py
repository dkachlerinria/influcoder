#!/usr/bin/env python3
"""hard_ratio sweep on the winning `ettin-encoder-400m` size, at the moredata
Counterfact scale (900 anchors / 2000 candidates / 500 held-out eval from the
same clean `NeelNanda/counterfact-tracing` pool, EPOCHS=8). IDENTICAL data
draw to `influcoder_attribute_noleak_counterfact_extquery_moredata_encodersweep.py`
(same SEED=0, same pool construction) -- the existing HARD_RATIO=0.5/400m
result (Recall@50=0.4609, MRR=0.8241, current best Recall@50 in the doc) is
NOT recomputed here; this only adds the 0.25 and 0.75 points, bracketing it.

Teacher gradients for this fixed anchor/candidate/eval draw are computed ONCE
and reused across both new hard_ratio values (per FINDINGS.md's documented
time-saver -- the targets matrix is a pure function of the data draw,
independent of hard_ratio, which only affects `distill()`'s negative-mining).

Usage:
    python methods/influcoder_attribute_noleak_counterfact_extquery_moredata_400m_hardsweep.py
"""
...
HARD_RATIOS = [0.25, 0.75]  # brackets the existing 0.5 result (0.4609 Recall@50)
...
def main():
    ...
    print(f"InfluCoder 400m hard_ratio sweep / DATE-LM | task={TASK} subset={SUBSET} | "
    ...
    print(f"external anchors={len(ext_anchor_idx)} candidates={len(teacher_idx)} "
          f"held-out eval={len(eval_idx)} (IDENTICAL draw to the encodersweep script -- "
          f"same SEED, same pool construction)")

    print(f"\n########## teacher gradients: external anchors/'queries' (n={len(ext_anchor_idx)}) "
          f"[computed ONCE, reused for every hard_ratio below] ##########")
    ...
    for hard_ratio in HARD_RATIOS:
        tag = f"hard{int(hard_ratio * 100):03d}"
        print(f"\n{'=' * 70}\ndistilling {ENCODER_MODEL} ({EPOCHS} epochs) hard_ratio={hard_ratio} "
              f"on EXTERNAL anchors x candidates only\n{'=' * 70}")
        ...
        save_path = Path(f"results/factual-attribution-influcoder-noleak-extquery-moredata-400m-{tag}/InfluCoder.pt")
        save_path.parent.mkdir(parents=True, exist_ok=True)
        with open(save_path, "w") as f:
            json.dump(final_scores, f)
        wall = time.perf_counter() - t_size0
        print(f"  wrote {save_path} (this hard_ratio's wall time: {wall:.1f}s)")
        summary[tag] = {"hard_ratio": hard_ratio, "save_path": str(save_path), "wall_s": wall}
