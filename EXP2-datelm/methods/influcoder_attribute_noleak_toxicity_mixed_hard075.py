#!/usr/bin/env python3
"""Single-variable hard_ratio ablation of the winning cross-source toxicity
mix. IDENTICAL to `influcoder_attribute_noleak_toxicity_mixed.py` (which
scored AUPRC 0.6160, the best result in this doc) except for ONE change:
`HARD_RATIO` 0.5 -> 0.75. Sample counts, epochs, seed, and every other
constant/piece of logic are byte-identical to that script.

This extends the hard_ratio curve established by the other two single-variable
ablations at this same (original, proven) scale:
  - HARD_RATIO=0.0 -> AUPRC 0.4664 (`influcoder_attribute_noleak_toxicity_mixed_nohard.py`)
  - HARD_RATIO=0.5 -> AUPRC 0.6160 (`influcoder_attribute_noleak_toxicity_mixed.py`, the winner)
  - HARD_RATIO=0.75 -> this script

0.75 was chosen over 1.0 deliberately: `FINDINGS.md` (EXP1, same
`influcoder.encoder.distill` code path) documented hard_ratio=1.0 as a hard
cliff, not a graceful falloff (collapsed to +0.474 vs. 0.75's +0.769 at the
same pool size in that context) -- pure hard negatives leave no easy contrast
for the ranking loss to calibrate against. 0.75 is the well-evidenced next
point to check between the proven 0.5 and the known-bad 1.0.

Role assignment (unchanged from the source script):
  - toxic anchors (100):      WildGuardMix (`response_harm_label=='harmful'`)
  - toxic candidates (300):   ToxicChat (`toxicity==1`)
  - toxic held-out eval (100): WildGuardMix, disjoint from the anchor slice
  - benign candidates/eval: UltraChat throughout

Usage:
    python methods/influcoder_attribute_noleak_toxicity_mixed_hard075.py
"""
...
MAX_LEN = 1024
PROJ_DIM = 8192
N_EXT_ANCHORS = 100     # WildGuardMix -- same count as the winning script
N_EXT_BENIGN = 1000     # UltraChat
N_EXT_TOXIC_POS = 300   # ToxicChat -- DIFFERENT source than the anchors
...
HARD_RATIO = 0.75       # ONLY CHANGE vs. the 0.6160 winning script (was 0.5) -- explicit instruction
LR = 5e-5
SEED = 0
SELECT_BEST_ON = "aggregated"

SAVE_PATH = Path("results/toxicity-bias-influcoder-noleak-mixed-hard075/InfluCoder.pt")
...
def main():
    random.seed(SEED)
    print(f"InfluCoder (leak-free, CROSS-SOURCE mixed, HARD_RATIO=0.75) / DATE-LM | task={TASK} subset={SUBSET} "
          f"| base_model={BASE_MODEL} checkpoint={CHECKPOINT}\n")
    print("toxic anchors <- WildGuardMix | toxic candidates <- ToxicChat | "
          "toxic held-out eval <- WildGuardMix (disjoint from anchors) | hard_ratio=0.75\n")
