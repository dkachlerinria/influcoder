#!/usr/bin/env python3
"""More-external-samples ablation of `influcoder_attribute_noleak_toxicity.py`
-- ONLY N_EXT_ANCHORS/N_EXT_BENIGN/N_EXT_TOXIC_POS/N_EVAL_BENIGN/N_EVAL_TOXIC
are raised; HARD_RATIO and EPOCHS are held fixed at their original values on
purpose. FINDINGS.md (EXP1, same distill() code path) found that scaling up
data WITHOUT proportionally raising hard_ratio can regress quality -- this run
deliberately does NOT compensate for that, to test whether the effect shows up
here too rather than assuming it transfers. See EXP2.md for the result.
...
MAX_LEN = 1024
PROJ_DIM = 8192
N_EXT_ANCHORS = 100     # moredata ablation: 2.5x the 40 used in the recommended run
N_EXT_BENIGN = 1000     # moredata ablation: ~3.3x the 300 used in the recommended run
N_EXT_TOXIC_POS = 300   # moredata ablation: 3x the 100 used in the recommended run
N_EVAL_BENIGN = 300     # moredata ablation: 3x the 100 used in the recommended run
N_EVAL_TOXIC = 100      # moredata ablation: 2x the 50 used in the recommended run
EPOCHS = 8              # held fixed -- FINDINGS.md: more epochs is a dead lever
HARD_RATIO = 0.5        # held fixed on purpose -- see module docstring
LR = 5e-5
SEED = 0
SELECT_BEST_ON = "aggregated"

SAVE_PATH = Path("results/toxicity-bias-influcoder-noleak-moredata/InfluCoder.pt")
