#!/usr/bin/env python3
"""Phase-B data-scale-up of `influcoder_attribute_noleak_counterfact_extquery_moredata.py`
(Recall@50=0.4411, MRR=0.8226 at 900/2000/500, HARD_RATIO=0.5). Run after a
hard_ratio sweep (0.0/0.25/0.5/0.75) at the moredata scale found only a small,
non-monotonic ~0.01 swing in Recall@50 -- not confidently a real effect, so
this instead doubles sample counts (900->1800, 2000->4000, 500->1000) at the
same HARD_RATIO=0.5, to test whether Recall@50 responds to more data instead.
Pool has 15,648 clean rows; this uses 6,800, still well within budget. See
EXP2.md for the result.
...
SAVE_PATH = Path("results/factual-attribution-influcoder-noleak-extquery-bigger/InfluCoder.pt")
