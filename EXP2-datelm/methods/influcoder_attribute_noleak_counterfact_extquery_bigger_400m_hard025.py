#!/usr/bin/env python3
"""Combines the two winning levers found for 400m Counterfact separately:
`hard_ratio=0.25` (moredata scale: Recall@50 0.4609->0.4689, MRR 0.8241->0.8739,
the single largest gain found in this whole campaign) and the bigger 1800/4000/1000
data scale (which alone, at hard_ratio=0.5, actually regressed slightly vs.
moredata: 0.4511 < 0.4609). Tests whether hard_ratio=0.25's gain is additive
with more data, since data-scale-alone was not a good lever on its own.
ONLY CHANGES vs. `influcoder_attribute_noleak_counterfact_extquery_bigger_400m.py`:
HARD_RATIO 0.5 -> 0.25. Same 1800/4000/1000 sample counts, EPOCHS=8, SEED=0.
See EXP2.md for the result.
...
HARD_RATIO = 0.25      # winning value found on the moredata-scale 400m sweep
LR = 5e-5
SEED = 0
SELECT_BEST_ON = "aggregated"

SAVE_PATH = Path("results/factual-attribution-influcoder-noleak-extquery-bigger-400m-hard025/InfluCoder.pt")
