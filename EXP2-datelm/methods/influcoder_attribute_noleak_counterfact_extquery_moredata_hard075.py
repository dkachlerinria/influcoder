#!/usr/bin/env python3
"""Single-variable hard_ratio ablation of `influcoder_attribute_noleak_counterfact_extquery_moredata.py`
(Recall@50=0.4411, MRR=0.8226, the moredata baseline) -- IDENTICAL except
HARD_RATIO 0.5 -> 0.75. Sample counts (900 anchors/2000 candidates/500 eval),
epochs, seed, and all other logic are byte-identical to that script. Part of
a hard_ratio sweep (0.0/0.25/0.5/0.75) testing whether Recall@50 -- which the
moredata sample-count increase alone did not move -- responds to hard-negative
mining instead. See EXP2.md for the result.
...
HARD_RATIO = 0.75      # ONLY CHANGE vs. the moredata baseline (was 0.5) -- sweep point
LR = 5e-5
SEED = 0
SELECT_BEST_ON = "aggregated"

SAVE_PATH = Path("results/factual-attribution-influcoder-noleak-extquery-moredata-hard075/InfluCoder.pt")
