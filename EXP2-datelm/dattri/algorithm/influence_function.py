...
from .base import BaseAttributor, BaseInnerProductAttributor, BaseCosineSimilarityAttributor
...
class IFAttributorArnoldi(BaseInnerProductAttributor):
    ...
    def cache(
        ...
        data_iter = iter(full_train_dataloader)
        data_target_pair_list = []
        for _ in range(iter_number):
            try:
                data_target_pair_list.append(next(data_iter))
...
class IFAttributorDataInf(BaseInnerProductAttributor):
    ...
    def cache(
        ...
        for checkpoint_idx in range(len(self.task.get_checkpoints())):
            ...
            for _ in range(iter_number):
                try:
                    sampled_data_list.append(next(data_iter))
                except StopIteration:
                    break  # End of data
            for sampled_data_ in tqdm(sampled_data_list):
                ...
                _cached_train_reps_list.append(sampled_data_rep.detach().cpu())
                del sampled_data, sampled_data_rep
    ...
    def transform_test_rep(
        ...
        with torch.no_grad():
            for layer in range(layer_cnt):
                ...
                grad_layer = grad_layer.to(test_rep_layers[layer].device)
                ...
                running_transformation = running_transformation.detach()
...
class IFAttributorLESS(BaseCosineSimilarityAttributor):
    """The inner product attributor with EK-FAC inverse FIM transformation."""
    def __init__(
        self,
        task: AttributionTask,
        ...
        self.full_train_dataloader = None
        self.projector = BasicProjector(grad_dim = grad_in_dim,
                                proj_dim=proj_dim,
                                seed=0,
    ...
    def prepare_optimizer_state(self):
        def prepare_optimizer_state(model,optimizer_state, device):
            ...
            return avg, avg_sq
        for checkpoint in self.task.checkpoints:
            if os.path.isdir(checkpoint):
                optimizer_path = os.path.join(checkpoint,"optimizer.bin")
                if not os.path.exists(optimizer_path):
                    error_msg = (
                        "LESS misses optimizer state"
                    )
                    raise ValueError(error_msg)
            else:
                try:
                    optimizer_path = hf_hub_download(repo_id=checkpoint, filename="optimizer.bin")
            ...
            self.optimizer_state_dict[checkpoint] = prepare_optimizer_state(self.task.model,optimizer_state,self.device)
    ...
    def transform_train_rep(
        ...
        beta2 = 0.999
        eps = 1e-08
        m, v = self.optimizer_state_dict[self.task.checkpoints[ckpt_idx]]
        updated_avg = beta1 * m + (1 - beta1) * train_rep
        updated_avg_sq = beta2 * v + (1 - beta2) * train_rep ** 2
        vectorized_grads = updated_avg / torch.sqrt(updated_avg_sq + eps)
        current_projected_grads = self.projector.project(
                vectorized_grads, model_id=0)
        return current_projected_grads
