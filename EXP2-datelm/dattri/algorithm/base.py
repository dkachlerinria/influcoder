    ...
    def attribute(
        ...
        for checkpoint_idx in range(len(self.task.get_checkpoints())):
            ...
            for test_batch_idx, test_batch_data_ in enumerate(
                    tqdm(
                        test_dataloader,
                        desc="calculating gradient of test set in...",
                        leave=False,
                    ),
                ):
                    ...
                    test_reps.append(test_batch_rep)
            ...
            for train_batch_idx, train_batch_data_ in enumerate(
                ...
                for test_batch_idx, _ in enumerate(
                    tqdm(
                        test_dataloader,
                        desc="Calculating Influence",
                        leave=False,
                    ),
                ):
                    ...
                    tda_output[row_st:row_ed, col_st:col_ed] += (
                        train_batch_rep @ test_reps[col_st:col_ed].T / denom.unsqueeze(-1)
                        if denom is not None
                        else train_batch_rep @ test_reps[col_st:col_ed].T
                    )
...
class BaseCosineSimilarityAttributor(BaseAttributor):
    """Base class for inner product attributors."""
    ...
    def attribute(
        ...
        def cosine_similarity(X: torch.Tensor, Y: torch.Tensor) -> torch.Tensor:
            X_norm = X / (X.norm(dim=1, keepdim=True, p=2) + 1e-8)
            Y_norm = Y / (Y.norm(dim=1, keepdim=True, p=2) + 1e-8)
        ...
        for checkpoint_idx in range(len(self.task.get_checkpoints())):
            test_reps = []
            tda_output *= checkpoint_idx
            # store validation gradients first
            for test_batch_idx, test_batch_data_ in enumerate(
                    tqdm(
                        test_dataloader,
                        desc="calculating gradient of test set in...",
            ...
            for train_batch_idx, train_batch_data_ in enumerate(
                ...
                for test_batch_idx, _ in enumerate(
                    ...
                    tda_output[row_st:row_ed, col_st:col_ed] += (
                        cosine_similarity(train_batch_rep,test_reps[col_st:col_ed])
                    )
